...
! defined( 'WPCPC_VERSION' ) && define( 'WPCPC_VERSION', '1.0.4' );
		...
		if ( ! class_exists( 'WPCleverWpcpc' ) && class_exists( 'WC_Product' ) ) {
			class WPCleverWpcpc {
				...
				function __construct() {
					...
					// ajax
					add_action( 'wp_ajax_wpcpc_search_term', [ $this, 'search_term' ] );
					add_action( 'wp_ajax_wpcpc_add_condition', [ $this, 'add_condition' ] );
					...
					add_action( 'before_woocommerce_init', function () {
						if ( class_exists( '\Automattic\WooCommerce\Utilities\FeaturesUtil' ) ) {
							FeaturesUtil::declare_compatibility( 'custom_order_tables', WPCPC_FILE );
						}
					} );
				}
				...
				function admin_menu_content() {
					...
					<div class="wpclever_settings_page wrap">
						<h1 class="wpclever_settings_page_title"><?php echo esc_html__( 'WPC Smart Product Collections', 'wpc-smart-product-collections' ) . ' ' . WPCPC_VERSION; ?></h1>
						<div class="wpclever_settings_page_desc about-text">
							<p>
								<?php printf( esc_html__( 'Thank you for using our plugin! If you are satisfied, please reward it a full five-star %s rating.', 'wpc-smart-product-collections' ), '<span style="color:#ffb900">&#9733;&#9733;&#9733;&#9733;&#9733;</span>' ); ?>
								<br/>
										...
										<tr>
											...
											<td>
												...
												<select name="wpcpc_settings[archive_position]" <?php echo esc_attr( $archive_position !== 'default' ? 'disabled' : '' ); ?>>
													...
													<option value="0" <?php echo esc_attr( ! $archive_position || ( $archive_position === '0' ) ? 'selected' : '' ); ?>><?php esc_html_e( 'None (hide it)', 'wpc-smart-product-collections' ); ?></option>
												</select>
											</td>
										</tr>
										<tr>
										...
										<tr>
											...
											<td>
												...
												<select name="wpcpc_settings[single_position]" <?php echo esc_attr( $single_position !== 'default' ? 'disabled' : '' ); ?>>
													...
													<option value="0" <?php echo esc_attr( ! $single_position || ( $single_position === '0' ) ? 'selected' : '' ); ?>><?php esc_html_e( 'None (hide it)', 'wpc-smart-product-collections' ); ?></option>
												</select>
											</td>
										</tr>
										<tr>
				...
				function condition( $key = '', $condition = [] ) {
					...
					$apply   = isset( $condition['apply'] ) ? $condition['apply'] : 'sale';
					$compare = isset( $condition['compare'] ) ? $condition['compare'] : 'is';
					$value   = isset( $condition['value'] ) ? (array) $condition['value'] : [];
					?>
				...
				function form_fields( $term = null ) {
					if ( $term ) {
						...
						$conditions  = get_term_meta( $term->term_id, 'wpcpc_conditions', true ) ? (array) get_term_meta( $term->term_id, 'wpcpc_conditions', true ) : [];
						$include     = get_term_meta( $term->term_id, 'wpcpc_include', true ) ? (array) get_term_meta( $term->term_id, 'wpcpc_include', true ) : [];
						$exclude     = get_term_meta( $term->term_id, 'wpcpc_exclude', true ) ? (array) get_term_meta( $term->term_id, 'wpcpc_exclude', true ) : [];
